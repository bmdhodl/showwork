...
python -m showwork require --session first-look --id config --scope artifact --description "config/api.yaml exists" --type file_exists --path config/api.yaml
...
Start a session, declare `require` before the first claim, let the Cursor
rule drive claims after each change, and close with `finish --status ok`.
If you delete a file you did not claim, verify goes RED. That is issue #64.
